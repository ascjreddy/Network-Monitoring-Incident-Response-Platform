...
def get_recent_events(seconds=600):
    conn = get_conn()
    cur  = conn.cursor()
...
def get_reachable_devices():
    ...
    cur.execute("""
        SELECT DISTINCT device_name
        FROM snmp_metrics
        WHERE timestamp > %s AND if_status = 'up'
    """, (since,))
    rows = [r[0] for r in cur.fetchall()]
    cur.close()
    conn.close()
...
def get_recently_resolved():
    """
    Returns a dict of {(device, type): resolved_at} for incidents
    resolved in the last 20 minutes. Used to ignore syslog events
    that are older than the resolution time.
    """
    conn = get_conn()
    cur  = conn.cursor()
    cur.execute("""
        SELECT root_cause_device, incident_type, resolved_at
        FROM incidents
        WHERE resolved = TRUE
        AND resolved_at > NOW() - INTERVAL '20 minutes'
    """)
...
def resolve_cleared_incidents(reachable_devices, events):
    conn = get_conn()
    cur  = conn.cursor()
    ...
    for device in reachable_devices:
        cur.execute("""
            ...
            AND incident_type = 'device_unreachable'
            AND resolved = FALSE
            RETURNING root_cause_device, incident_type
        """, (device,))
        for row in cur.fetchall():
            print(f"\n{'='*60}")
            print(f"  INCIDENT RESOLVED -- {datetime.now()}")
            print(f"{'='*60}")
            print(f"  Device:  {row[0]}")
            print(f"  Type:    {row[1].upper()}")
            print(f"  Status:  Device is responding to SNMP again")
            print(f"{'='*60}\n")

    for ts, device, sev, msg in events:
        if "LINEPROTO-5-UPDOWN" in msg and "up" in msg.lower() and "down" not in msg.lower():
            cur.execute("""
                UPDATE incidents
                SET resolved = TRUE, resolved_at = NOW()
                WHERE root_cause_device = %s
                AND incident_type = 'interface_down'
                AND resolved = FALSE
                RETURNING root_cause_device, incident_type
            """, (device,))
            for row in cur.fetchall():
                ...
                print(f"  Device:  {row[0]}")
                print(f"  Type:    {row[1].upper()}")
                print(f"  Status:  Interface is back up")
                print(f"{'='*60}\n")

    for ts, device, sev, msg in events:
        if "OSPF" in msg and "ADJCHG" in msg and "FULL" in msg and "DOWN" not in msg.upper():
            cur.execute("""
                UPDATE incidents
                SET resolved = TRUE, resolved_at = NOW()
                WHERE root_cause_device = %s
                AND incident_type = 'ospf_neighbor_lost'
                AND resolved = FALSE
                RETURNING root_cause_device, incident_type
            """, (device,))
            for row in cur.fetchall():
                ...
                print(f"  Device:  {row[0]}")
                print(f"  Type:    {row[1].upper()}")
                print(f"  Status:  OSPF adjacency restored")
                print(f"{'='*60}\n")
...
def detect_incidents(events, unreachable, recently_resolved):
    incidents = []

    for ts, device, sev, msg in events:
        if "LINEPROTO-5-UPDOWN" in msg and "down" in msg.lower():
            key = (device, "interface_down")
            # THE FIX: skip if this event is OLDER than the last resolution
            if key in recently_resolved:
                resolved_at = recently_resolved[key]
                # Make ts timezone-naive for comparison
                ts_naive = ts.replace(tzinfo=None) if ts.tzinfo else ts
                resolved_naive = resolved_at.replace(tzinfo=None) if resolved_at.tzinfo else resolved_at
                if ts_naive <= resolved_naive:
                    continue
            iface = "unknown"
    ...
    for ts, device, sev, msg in events:
        if "OSPF" in msg and "ADJCHG" in msg and "DOWN" in msg.upper():
            key = (device, "ospf_neighbor_lost")
            # THE FIX: skip if this event is OLDER than the last resolution
            if key in recently_resolved:
                resolved_at = recently_resolved[key]
                ts_naive = ts.replace(tzinfo=None) if ts.tzinfo else ts
    ...
    for ts, device, sev, msg in events:
        if "LINEPROTO-5-UPDOWN" in msg or "LINK-3-UPDOWN" in msg:
            flap_counts[device] = flap_counts.get(device, 0) + 1

    for device, count in flap_counts.items():
        if count >= 5:
            key = (device, "link_flapping")
            if key in recently_resolved:
                continue
            incidents.append({
                "type": "link_flapping",
...
def build_timeline(events, device, start_ts):
    ...
    for ts, dev, sev, msg in events:
        ...
        if abs((ts_naive - start_naive).total_seconds()) < 120:
...
def run_engine():
    ...
    while True:
        try:
            ...
            recently_resolved = get_recently_resolved()

            resolve_cleared_incidents(reachable, events)

            incidents = detect_incidents(events, unreachable, recently_resolved)
            if incidents:
                for inc in incidents:
                    save_incident(inc)
            else:
                print(f"[{datetime.now()}] No incidents detected -- network healthy")
